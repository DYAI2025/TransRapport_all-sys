...
class TestDocsCrossRefContract:
    ...
    def test_docs_crossref_basic_help(self):
        """Test that 'transrapport-docs docs cross-ref --help' works."""
        result = subprocess.run(
            ["transrapport-docs", "docs", "cross-ref", "--help"],
            capture_output=True,
            text=True,
        )
    ...
    def test_docs_crossref_basic_execution(self):
        """Test that 'transrapport-docs docs cross-ref' executes without error."""
        result = subprocess.run(
    ...
    def test_docs_crossref_term_flag(self):
        """Test that 'transrapport-docs docs cross-ref --term ATO' accepts the term flag."""
        result = subprocess.run(
    ...
    def test_docs_crossref_file_flag(self):
        """Test that 'transrapport-docs docs cross-ref --file <file>' accepts the file flag."""
        result = subprocess.run(
            [
                "transrapport-docs",
                "docs",
    ...
    def test_docs_crossref_json_format(self):
        """Test that 'transrapport-docs docs cross-ref --format json' produces JSON output."""
        result = subprocess.run(
